...
from dinox_node import DinoxDetectorNode


@pytest.fixture
def mock_client():
    with patch("dinox_node.Client") as mock:
...
@pytest.fixture
def mock_config():
    with patch("dinox_node.Config") as mock:
...
@patch("dinox_node.DetectionTask")
def test_detect_and_annotate(
    ...
    with patch("dinox_node.DetectionTask.rle2mask") as mock_rle2mask:
        with patch("dinox_node.DetectionTask.string2rle") as mock_string2rle:
